...
HB_FUNC( FREE_GETCOPYRIGHTMESSAGE )
{
...
HB_FUNC( FREE_GETFILETYPEFROMMEMORY )
{
	if( HB_ISCHAR( 1 ) )
	{
...
HB_FUNC( FREE_GETTRANSPARENCYCOUNT )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_GETTRANSPARENCYTABLE )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_SETTRANSPARENCYTABLE )
{
	if( hb_FIBITMAP_is( 1 ) &&
		 HB_ISPOINTER( 2 ) &&
		 HB_ISNUM( 3 ) )
...
HB_FUNC( FREE_HASBACKGROUNDCOLOR )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_GETBACKGROUNDCOLOR )
{
	if( hb_FIBITMAP_is( 1 ) )
	{
		FIBITMAP * dib = hb_FIBITMAP_par( 1 );
...
HB_FUNC( FREE_SETBACKGROUNDCOLOR )
{
	if( hb_FIBITMAP_is( 1 ) &&
...
HB_FUNC( FREE_DESTROYICCPROFILE )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_CONVERTTOGREYSCALE )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_CONVERTTO16BITS555 )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_CONVERTTO16BITS565 )
{
	if( hb_FIBITMAP_is( 1 ) )
...
HB_FUNC( FREE_CONVERTTOSTANDARDTYPE )
{
	if( hb_FIBITMAP_is( 1 ) )
	{
		FIBITMAP * dib = hb_FIBITMAP_par( 1 );
...
HB_FUNC_TRANSLATE( FREE_ROTATECLASSIC, FREE_ROTATE )
...
HB_FUNC( FREE_MAKETHUMBNAIL )
{
	...
	{
		FIBITMAP * dib   = hb_FIBITMAP_par( 1 );
		int	  nPixel = hb_parni( 2 );
		BOOL	  lConvert = HB_ISLOG( 3 ) ? hb_fi_parl( 3 ) : TRUE;

		hb_FIBITMAP_ret( FreeImage_MakeThumbnail( dib, nPixel, lConvert ), HB_TRUE );
	}
	else
		hb_errRT_BASE_SubstR( EG_ARG, 0, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );
}
